...
class AttackAlgorithm(AttackAlgorithmBase):
    def __init__(self, config=None):
        ...
        self.replay_frac = float(c.get("replay_frac", 0.96))
        ...
        self.slow_k = int(c.get("slow_k", 10))  # gpt_oss multipost size
    ...
    def run(self, env, config):
        ...
        def make_msg(i):
            return _forge_plan(i, self.slow_k) if slow else TEMPLATE.format(url=_url(i), payload=PAYLOAD)
